...
async def process_generation_task(
    ...
    try:
        ...
        logger.warning("Resume generation failed for job=%s: %s", task.job_id, e)
        await db.execute(
            update(GenerationQueue)
            .where(GenerationQueue.id == task.id)
...
async def boot_scan(db: AsyncSession) -> None:
    ...
    result = await db.execute(
        ...
        .where(
            ...
            TailoredResume.id.is_(None),
        )
    )
    rows = result.all()
    if not rows:
        logger.info("Boot scan: all tracked jobs already have resumes")
        return
