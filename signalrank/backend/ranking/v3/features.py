...
def title_similarity(job: dict, profile: ProfileV3) -> float:
    ...
    if not job_title or not profile.target_roles:
        return 0.0
    best = max(
        token_set_ratio(job_title, _normalize(role)) for role in profile.target_roles
    )
    return best / 100.0
...
def role_family_match(job: dict, profile: ProfileV3) -> float:
    """1.0 if job description contains any active lane's must_have_terms. [0, 1]"""
    if not profile.active_lanes:
        return 0.0
    text = _text(job)
    for lane_name in profile.active_lanes:
        lane = LANE_REGISTRY.get(lane_name)
        if lane and any(term.lower() in text for term in lane.must_have_terms):
            return 1.0
    return 0.0
