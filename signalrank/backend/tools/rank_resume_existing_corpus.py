...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--corpus", dest="corpus", default=None, help="path to JSON corpus file (used by RANKING_ENGINE=v3)")
    parser.add_argument("--candidate-name", dest="candidate_name", default="", help="candidate name override (used by RANKING_ENGINE=v3)")
    parser.add_argument("--current-focus", dest="current_focus", default=None, help="current focus/role hint (used by RANKING_ENGINE=v3)")
    return parser
...
async def main_async() -> None:
    ...
    import os as _os
    _ranking_engine = _os.environ.get("RANKING_ENGINE", "v2").strip().lower()
    if _ranking_engine == "v3":
        import json as _json
        import sys as _sys
        _sys.path.insert(0, str(Path(__file__).resolve().parents[1]))
        from ranking.v3.pipeline import rank_jobs_v3
        _resume_text = _load_resume_text(Path(args.resume).expanduser().resolve())
        _corpus_path = args.corpus
        _jobs = _json.loads(Path(_corpus_path).read_text()) if _corpus_path else []
        _results = rank_jobs_v3(
            _resume_text, _jobs,
