...
APPROACHES = {
    "deterministic_baseline": ApproachSpec(
        "deterministic_baseline", "Deterministic Baseline", False, False
    ),
    ...
    "llm_parse_plus_agentic": ApproachSpec(
        "llm_parse_plus_agentic", "LLM Parse + Agentic", True, True
    ),
}
...
def _dedupe_strs(
    values: list[str] | tuple[str, ...] | None, *, limit: int | None = None
) -> list[str]:
    seen: set[str] = set()
    result: list[str] = []
...
def _extract_locations(resume_text: str) -> list[str]:
    ...
    explicit_specific_locations = [
        loc for loc in explicit_locations if loc not in {"India", "Remote"}
    ]
    if explicit_specific_locations:
        locations = explicit_specific_locations
    else:
        strong_locations = [
            canonical for score, _, canonical in scored_locations if score >= 3
        ]
        locations = _dedupe_strs(
            strong_locations or [canonical for _, _, canonical in scored_locations],
            limit=4,
        )
...
INNOVATION_DRIFT_PENALTIES = {
    ...
    "application developer": 10.0,
    "support engineer": 12.0,
    "technical support": 12.0,
    "customer support": 10.0,
    "qa engineer": 16.0,
    "quality assurance": 16.0,
    "quality automation": 16.0,
    "test engineer": 14.0,
    "manual testing": 8.0,
    "full stack": 18.0,
    "fullstack": 18.0,
    "site reliability": 18.0,
    "sre": 18.0,
}

NETWORK_DIRECT_TITLE_MARKERS = (
...
INNOVATION_GENERIC_TITLE_MARKERS = (
    "application developer",
    "backend",
    "data architect",
    "data engineer",
...
HARD_NEGATIVE_TITLE_PHRASES = {
    "customer support": 22.0,
    "delivery manager": 22.0,
    "engineering manager": 28.0,
    "helpdesk": 22.0,
    "operations manager": 22.0,
    "program manager": 22.0,
    "project manager": 22.0,
    "qa": 22.0,
    "quality assurance": 22.0,
    "quality automation": 22.0,
    "quality engineer": 22.0,
    "support engineer": 22.0,
    "technical support": 22.0,
    "test engineer": 22.0,
}
...
def _infer_seniority_band(years: int | None, text: str) -> str:
    ...
    if years is not None:
        ...
        return "principal"
    if any(
        term in lower for term in ("principal", "staff", "head", "architect", "lead")
    ):
        return "lead"
    if any(term in lower for term in ("senior", "sr.", "sr ")):
        return "senior"
    if any(
        term in lower for term in ("junior", "jr.", "fresher", "graduate", "intern")
    ):
        return "entry"
    return "mid"
...
def _infer_role_candidates(
    resume_text: str, skills: list[str], domains: list[dict[str, Any]], seniority: str
) -> list[str]:
    matches = _extract_phrase_matches(resume_text, ROLE_PHRASES)
    innovation_skills = {skill.lower() for skill in skills}
    if matches:
        if "iot" in innovation_skills and not any(
            "iot" in item.lower() for item in matches
        ):
            matches.append(
                "IoT Solutions Lead"
        ...
        if {
            ...
            "3d printing",
            "robotics",
        } & innovation_skills and not any(
            "prototype" in item.lower() or "creative technologist" in item.lower()
            for item in matches
        ):
    ...
    if (
        ...
        or {"iot", "robotics", "3d printing", "rapid prototyping", "conversational ai"}
        & innovation_skills
    ):
        candidates = [
            (
                "Innovation Lead"
                if seniority in {"lead", "principal"}
                else "Innovation Consultant"
            ),
            (
                "IoT Solutions Lead"
                if "iot" in innovation_skills and seniority in {"lead", "principal"}
                else "IoT Engineer"
            ),
            "Creative Technologist",
            "Prototype Engineer",
    ...
    if seniority in {"senior", "lead", "principal"}:
        candidates.append(
            f"Lead {keywords[0].title()} Engineer" if keywords else "Lead Engineer"
        )
    if not candidates:
...
def _build_target_roles(
    ...
    fallback_roles = _infer_role_candidates(
        " ".join(recent_titles), skills, domains, seniority
    )
...
def _build_archetypes(
    domains: list[dict[str, Any]], skills: list[str], seniority: str
) -> list[dict[str, Any]]:
    ...
    for idx, domain in enumerate(domains[:4]):
        ...
        archetypes.append(
            {
                "id": re.sub(r"[^a-z0-9]", "_", domain_name.lower()).strip("_")
                or "general",
                "label": domain_name,
                "priority": "primary" if idx == 0 else "secondary",
...
def _resume_supports_phrase(
    resume_text: str, phrase: str, alias_index: dict[str, list[str]] | None = None
) -> bool:
    ...
    return bool(phrase_tokens) and len(phrase_tokens & resume_tokens) >= max(
        1, min(2, len(phrase_tokens))
    )


def _resume_support_score(
    resume_text: str, phrase: str, alias_index: dict[str, list[str]] | None = None
) -> int:
    normalized_resume = _normalize_text_for_match(resume_text)
    normalized_phrase = _normalize_text_for_match(phrase)
    ...
    for alias in _dedupe_strs(aliases):
        ...
        if normalized_alias and normalized_alias in normalized_resume:
            best = max(best, 3)
    phrase_tokens = {
        token for token in _token_set(phrase) if token not in GENERIC_ROLE_TOKENS
    }
    resume_tokens = _token_set(resume_text)
...
def _expand_role_families(roles: list[str], seniority: str) -> list[str]:
    ...
    for role in roles:
        ...
        if not expansions and any(
            marker in normalized for marker in ENGINEERING_ROLE_MARKERS
        ):
            expansions = ["Software Engineer"]
            if (
                "ai" in normalized
                or "machine learning" in normalized
                or "mlops" in normalized
            ):
...
def _should_allow_engineering_broadening(profile: dict[str, Any]) -> bool:
    ...
    suggested_roles = [
        str(item)
        for item in (profile.get("suggested_roles") or [])
        if str(item).strip()
    ]
    recent_titles = [
        str(item) for item in (profile.get("recent_titles") or []) if str(item).strip()
    ]
    role_pool = _dedupe_strs(
        primary_roles + target_roles + suggested_roles + recent_titles, limit=12
    )
...
def _build_keyword_retrieval_queries(
    signals: dict[str, Any], plan: dict[str, Any]
) -> list[str]:
    ...
    skills = _dedupe_strs(
        list(signals.get("must_have_skills") or [])
        + list(signals.get("supporting_skills") or []),
        limit=12,
    )
    ...
    if any(
        item in combined_context for item in ("prototype", "prototyping", "3d printing")
    ):
        contexts.append("Rapid Prototyping")
    if any(
        item in combined_context
        for item in ("hardware", "esp32", "raspberry pi", "arduino")
    ):
        contexts.append("Hardware")
    if "iot" in combined_context or any(
        item in combined_context
        for item in ("mqtt", "esp32", "raspberry pi", "arduino")
    ):
        contexts.append("IoT")
    if "conversational ai" in combined_context or any(
        item in combined_context
        for item in ("dialogflow", "alexa skills", "bot framework")
    ):
        contexts.append("Conversational AI")
    contexts = _dedupe_strs(
        contexts
        + [
            value
            for value in INNOVATION_CONTEXT_KEYWORDS
            if value.lower() in combined_context
        ],
        limit=6,
    )
    ...
    high_signal_skills = [
        skill
        for skill in skills
        if skill in INNOVATION_SKILL_PRIORITIES
        or skill
        in {"IoT", "Conversational AI", "Rapid Prototyping", "3D Printing", "Robotics"}
    ]
    high_signal_skills = _dedupe_strs(high_signal_skills, limit=4)
    consultant_lane = any(
        "consultant" in _normalize_text(role) for role in role_context
    )
...
def _evaluate_search_terms(
    profile: dict[str, Any], queries: list[str]
) -> dict[str, Any]:
    signals = dict(profile.get("matching_signals") or {})
    primary_roles = {
        str(item).lower()
        for item in (signals.get("primary_roles") or [])
        if str(item).strip()
    }
    adjacent_roles = {
        str(item).lower()
        for item in (signals.get("adjacent_roles") or [])
        if str(item).strip()
    }
    broadened_roles = {
        str(item).lower()
        for item in (signals.get("broadened_roles") or [])
        if str(item).strip()
    }
    must_skills = {
        str(item).lower()
        for item in (signals.get("must_have_skills") or [])
        if str(item).strip()
    }
    supporting_skills = {
        str(item).lower()
        for item in (signals.get("supporting_skills") or [])
        if str(item).strip()
    }
    ...
    for query in queries:
        lowered = query.lower()
        has_role = any(
            role in lowered for role in primary_roles | adjacent_roles | broadened_roles
        )
        ...
        if has_role and has_skill:
            ...
            keyword_queries += 1
        if query not in set(signals.get("primary_roles") or []) and query not in set(
            signals.get("adjacent_roles") or []
        ):
...
def _filter_llm_hints(hints: dict[str, Any], resume_text: str) -> dict[str, Any]:
    ...
    filtered["search_queries"] = [
        ...
        if len((_token_set(str(item)) - GENERIC_ROLE_TOKENS) & _token_set(resume_text))
        >= 2
    ]
    filtered["negative_keywords"] = _dedupe_strs(
        hints.get("negative_keywords") or [], limit=8
    )
    return filtered
...
def _build_matching_signals(profile: dict[str, Any]) -> dict[str, Any]:
    target_roles = [
        item for item in (profile.get("target_roles") or []) if isinstance(item, dict)
    ]
    primary_roles = [
        str(item.get("title") or "")
    ...
    if not primary_roles:
        primary_roles = _dedupe_strs(
            list(profile.get("suggested_roles") or []), limit=2
        )
    supporting_roles = _dedupe_strs(
        adjacent_roles + list(profile.get("recent_titles") or []), limit=4
    )
    broadened_roles = []
    if _should_allow_engineering_broadening(profile):
        broadened_roles = _expand_role_families(
            primary_roles + supporting_roles,
            str(profile.get("seniority_band") or "mid"),
        )
    else:
        broadened_roles = _expand_role_families(
            primary_roles, str(profile.get("seniority_band") or "mid")
        )
    role_context = _dedupe_strs(
        primary_roles + supporting_roles + broadened_roles, limit=12
    )
    skills = _prioritize_skills(
        _dedupe_strs(list(profile.get("skills") or []), limit=20), role_context
    )
    prioritized_must_have_terms = _prioritize_skills(
        _dedupe_strs(list(profile.get("must_have_terms") or []), limit=20), role_context
    )
    must_have_skills = _dedupe_strs(prioritized_must_have_terms + skills[:6], limit=8)
    supporting_skills = [
        skill for skill in skills if skill not in set(must_have_skills)
    ][:8]
    domains = _dedupe_strs(
        [
            str(item.get("name") or "")
    ...
    exclusions = _dedupe_strs(
        list(profile.get("avoid_terms") or profile.get("suggested_exclusions") or []),
        limit=10,
    )
...
def _normalize_verification_results(
    raw: Any, top_jobs: list[dict[str, Any]]
) -> list[dict[str, Any]]:
...
def _dedupe_with_audit(
    candidates: list[str], *, limit: int | None = None
...
def _build_deterministic_queries(
    profile: dict[str, Any], *, max_terms: int = 12
) -> tuple[list[str], dict[str, list[str]]]:
    ...
    must_skills = _dedupe_strs(
        list(signals.get("must_have_skills") or [])
        + list(plan.get("skill_queries") or []),
        limit=4,
    )
    ...
    if must_skills:
        role_skill_count = sum(
            ...
            for query in queries
            if any(
                role.lower() in query.lower()
                for role in primary_roles + adjacent_roles + broadened_roles
            )
            and any(skill.lower() in query.lower() for skill in must_skills)
        )
        if role_skill_count < 4:
            supplemental: list[str] = []
            for role in _dedupe_strs(
                primary_roles + adjacent_roles + broadened_roles, limit=6
            ):
                for skill in must_skills[:3]:
                    candidate = f"{role} {skill}"
                    if candidate not in queries:
                        supplemental.append(candidate)
            combined, refill_audit = _dedupe_with_audit(
                queries + supplemental, limit=max_terms
            )
            queries = combined
...
def _extract_generic_profile(resume_text: str, resume_name: str = "") -> dict[str, Any]:
    ...
    recent_titles = _dedupe_strs(
        ...
        + [
            ...
            for line in lines
            if len(line.split()) <= 8
            and any(term in line.lower() for term, _ in ROLE_PHRASES)
        ],
        limit=3,
    )
    if not recent_titles:
        recent_titles = _infer_role_candidates(resume_text, skills, domains, seniority)[
            :3
        ]

    suggested_roles = _dedupe_strs(
        recent_titles + _infer_role_candidates(resume_text, skills, domains, seniority),
        limit=5,
    )
    ...
    target_roles = _build_target_roles(
        recent_titles or suggested_roles[:2], domains, skills, seniority
    )
    query_plan = {
        "title_queries": _dedupe_strs(suggested_roles + recent_titles, limit=5),
        "skill_queries": _dedupe_strs(skills, limit=5),
        "domain_queries": _dedupe_strs(
            [str(item.get("name") or "") for item in domains if isinstance(item, dict)],
            limit=5,
        ),
...
async def _llm_parse_resume(
    resume_text: str, client: httpx.AsyncClient, model_hint: str
) -> dict[str, Any]:
...
def _merge_list(
    base: list[str] | None, additions: list[str] | None, *, limit: int | None = None
) -> list[str]:
    result = list(base or [])
...
def _apply_llm_suggestions(
    profile: dict[str, Any], hints: dict[str, Any]
) -> dict[str, Any]:
    updated = dict(profile)
    updated["suggested_roles"] = _merge_list(
        updated.get("suggested_roles"), hints.get("role_phrases"), limit=6
    )
    updated["skills"] = _merge_list(
        updated.get("skills"), hints.get("skill_highlights"), limit=20
    )
    ...
    updated["industries"] = _merge_list(
        updated.get("industries"), hints.get("industry_labels"), limit=5
    )
    query_plan = dict(updated.get("query_plan") or {})
    query_plan["title_queries"] = _merge_list(
        query_plan.get("title_queries"), hints.get("role_phrases"), limit=6
    )
    query_plan["skill_queries"] = _merge_list(
        query_plan.get("skill_queries"), hints.get("skill_highlights"), limit=6
    )
    query_plan["domain_queries"] = _merge_list(
        query_plan.get("domain_queries"), hints.get("domain_labels"), limit=5
    )
    query_plan["negative_keywords"] = _merge_list(
        query_plan.get("negative_keywords"), hints.get("negative_keywords"), limit=8
    )
    query_plan["search_queries"] = _merge_list(
        query_plan.get("search_queries"), hints.get("search_queries"), limit=8
    )
    updated["query_plan"] = query_plan
    updated["suggested_search_queries"] = _merge_list(
        updated.get("suggested_search_queries"),
...
async def _enrich_profile_with_llm(
    profile: dict[str, Any], resume_text: str
) -> dict[str, Any]:
...
def _job_vector(job: JobRecord) -> Counter:
    return Counter(
        _extract_terms(f"{job.title} {job.description} {job.company} {job.location}")
    )
...
def _infer_job_seniority(title: str, description: str) -> str:
    ...
    if any(
        term in text for term in ("principal", "staff", "head", "architect", "lead")
    ):
        return "lead"
...
def _role_phrase_allows_title_phrase(role_phrases: list[str], phrase: str) -> bool:
    phrase = _normalize_text_for_match(phrase)
    if not phrase:
        return False
    if phrase in HARD_NEGATIVE_TITLE_PHRASES and "manager" in phrase:
        return any(phrase in role for role in role_phrases)
    if "support" in phrase:
        return any("support" in role for role in role_phrases)
    if phrase == "qa" or "quality" in phrase or "test" in phrase:
        return any(
            marker in role
            for role in role_phrases
            for marker in ("qa", "quality", "sdet", "test")
        )
    return any(phrase in role for role in role_phrases)


def _hard_negative_title_penalty(title: str, role_phrases: list[str]) -> float:
    normalized_title = _normalize_text_for_match(title)
    penalty = 0.0
    for phrase, value in HARD_NEGATIVE_TITLE_PHRASES.items():
        if phrase not in normalized_title:
            continue
        if _role_phrase_allows_title_phrase(role_phrases, phrase):
            continue
        penalty = max(penalty, value)
    if "manager" in normalized_title and not any(
        "manager" in role for role in role_phrases
    ):
...
def _network_noise_penalty(title: str, combined_text: str) -> float:
    title_text = _normalize_text_for_match(title)
    direct_title = _contains_marker(title_text, NETWORK_DIRECT_TITLE_MARKERS)
    evidence = _contains_marker(combined_text, NETWORK_EVIDENCE_MARKERS)
    generic_title = _contains_marker(title_text, NETWORK_GENERIC_TITLE_MARKERS)
    if direct_title:
        return 0.0
    if generic_title:
        return 32.0
...
def _innovation_noise_penalty(title: str, combined_text: str) -> float:
    title_text = _normalize_text_for_match(title)
    generic_title = _contains_marker(title_text, INNOVATION_GENERIC_TITLE_MARKERS)
    if not generic_title:
        return 0.0
    if _contains_marker(title_text, INNOVATION_EVIDENCE_MARKERS):
        return 0.0
    return 24.0


def _score_jobs(
    resume_profile: dict[str, Any], jobs: list[JobRecord], *, agentic: bool
) -> list[dict[str, Any]]:
    signals = dict(
        resume_profile.get("matching_signals")
        or _build_matching_signals(resume_profile)
    )
    weights = dict(DEFAULT_SCORE_WEIGHTS)
    ...
    profile_terms = Counter(
        _extract_terms(
            " ".join(
                ...
                + [
                    str(item.get("name") or "")
                    for item in (resume_profile.get("domains") or [])
                    if isinstance(item, dict)
                ]
    ...
    role_terms = [
        term
        for term in _extract_terms(
            " ".join(
                signals.get("primary_roles") or signals.get("adjacent_roles") or []
            )
        )
    ...
    skill_terms = [
        ...
        for term in _extract_terms(
            " ".join(
                (signals.get("must_have_skills") or [])
                + (signals.get("supporting_skills") or [])
            )
        )
        if term not in STOPWORDS
    ]
    location_terms = [
        str(loc).lower()
        for loc in (signals.get("preferred_locations") or [])
        if str(loc).strip()
    ]
    avoid_terms = [
        term.lower() for term in (signals.get("avoid_terms") or []) if str(term).strip()
    ]
    profile_band = str(
        signals.get("seniority_band")
        or resume_profile.get("seniority_band")
        or _infer_seniority_band(
            resume_profile.get("years_of_experience"),
            " ".join(resume_profile.get("suggested_roles") or []),
        )
    ).lower()
    curated_terms = [
        term
        for term in _extract_terms(" ".join(signals.get("curated_queries") or []))
        if term not in STOPWORDS
    ]
    curated_phrases = [
    ...
    primary_role_phrases = [
        _normalize_text_for_match(value)
        for value in (signals.get("primary_roles") or [])
        if str(value).strip()
    ]
    adjacent_role_phrases = [
        _normalize_text_for_match(value)
        for value in (signals.get("adjacent_roles") or [])
        if str(value).strip()
    ]
    must_skill_terms = [
        _normalize_text_for_match(value)
        for value in (signals.get("must_have_skills") or [])
        if str(value).strip()
    ]
    supporting_skill_terms = [
        _normalize_text_for_match(value)
        for value in (signals.get("supporting_skills") or [])
        if str(value).strip()
    ]
    domain_terms = [
        _normalize_text_for_match(value)
        for value in (signals.get("domain_terms") or [])
        if str(value).strip()
    ]
    industry_terms = [
        _normalize_text_for_match(value)
        for value in (signals.get("industry_terms") or [])
        if str(value).strip()
    ]
    signal_text = " ".join(
        [
    ...
    sap_profile = any(
        ...
        for marker in ("sap", "s/4hana", "s4hana", "order to cash", "otc")
    )
    sap_specific_profile = any(
        marker in signal_text for marker in ("sap sd", "order to cash", "otc")
    )
    network_automation_profile = any(
        marker in signal_text
        for marker in (
            "cloud network",
            "firewall",
    ...
    innovation_profile = any(
        _is_innovation_role(value)
        for value in (signals.get("primary_roles") or [])
        + (signals.get("adjacent_roles") or [])
    )
    ...
    for i, job in enumerate(jobs):
        ...
        overlap_terms = sorted(
            set(role_terms + skill_terms)
            & set(_extract_terms(f"{job.title} {job.description}"))
        )
        location_score = (
            50.0
            if not location_terms
            or any(loc in job.location.lower() for loc in location_terms)
            else 15.0
        )
        recency_score = _derive_recency_score(job.date_posted)
        title_relevance = min(
            100.0, 18.0 * title_hits + 7.0 * skill_hits + 4.0 * len(overlap_terms)
        )
        seniority_score = 100.0 * _score_seniority_match(
            profile_band, _infer_job_seniority(job.title, job.description)
        )
        negative_penalty = sum(
            8.0 for term in avoid_terms if term and (term in title or term in desc)
        )
        ...
        hard_title_penalty = _hard_negative_title_penalty(
            job.title,
            primary_role_phrases,
        )
        negative_penalty += hard_title_penalty

        primary_role_hits = sum(
            1 for phrase in primary_role_phrases if phrase and phrase in combined_text
        )
        adjacent_role_hits = sum(
            1 for phrase in adjacent_role_phrases if phrase and phrase in combined_text
        )
        must_skill_hits = sum(
            1 for phrase in must_skill_terms if phrase and phrase in combined_text
        )
        supporting_skill_hits = sum(
            1 for phrase in supporting_skill_terms if phrase and phrase in combined_text
        )
        domain_hits = sum(
            1 for phrase in domain_terms if phrase and phrase in combined_text
        )
        industry_hits = sum(
            1 for phrase in industry_terms if phrase and phrase in combined_text
        )
        domain_role_boost = 0.0
        domain_mismatch_penalty = 0.0
        if innovation_profile:
            domain_mismatch_penalty = max(
                domain_mismatch_penalty,
                _innovation_noise_penalty(job.title, combined_text),
            )
        if sap_profile and "sap" in combined_text:
            sap_direct_job = any(
            ...
            ) or ("gts" in combined_text and "sd" in combined_text)
            if sap_direct_job:
                domain_role_boost = max(domain_role_boost, 28.0)
            elif "consultant" in title and not sap_specific_profile:
                domain_role_boost = max(domain_role_boost, 18.0)
            elif sap_specific_profile:
                domain_mismatch_penalty = max(domain_mismatch_penalty, 18.0)
            if sap_specific_profile and any(
                module in title for module in ("sap bw", "sap mm", "sap pp", "sap qm")
            ):
                if "sap sd" not in title and "otc" not in title:
                    domain_mismatch_penalty = max(domain_mismatch_penalty, 18.0)
        title_has_network = "network" in title
        if network_automation_profile:
            domain_mismatch_penalty = max(
                domain_mismatch_penalty,
                _network_noise_penalty(job.title, combined_text),
            )
        if network_automation_profile and title_has_network:
            if any(
                marker in combined_text
                for marker in (
                    "automation",
                ...
                domain_role_boost = max(domain_role_boost, 24.0)
            elif "network engineer" in title:
                domain_role_boost = max(domain_role_boost, 14.0)
        curated_token_hits = sum(
            1 for term in curated_terms if term in title or term in desc
        )
        curated_phrase_hits = sum(
            1 for phrase in curated_phrases if phrase and phrase in combined_text
        )
        base = (
        ...
        if agentic:
            ...
            if evidence_hits >= 6:
                ...
                base -= 2.5
        if hard_title_penalty:
            base = min(base, 39.0)
        if domain_mismatch_penalty:
            base = min(base, 49.0)
        if domain_mismatch_penalty >= 24.0:
            base = min(base, 39.0)

        fit_band = (
            "strong_fit" if base >= 58 else "weak_fit" if base >= 42 else "reject"
        )
        ...
        scores.append(
            {
                ...
                "domain_role_boost_score": float(domain_role_boost),
                "domain_mismatch_penalty": float(domain_mismatch_penalty),
                "fit_band": fit_band,
        ...
        if cached_payload:
            try:
                fetched_at = datetime.fromisoformat(
                    str(cached_payload.get("fetched_at") or "").replace("Z", "+00:00")
                )
            except Exception:
                fetched_at = None
            cached_hours = int(cached_payload.get("hours_old") or 0)
            cached_jobs = [
                _jobrecord_from_dict(item)
                for item in (cached_payload.get("jobs") or [])
                if isinstance(item, dict)
            ]
    ...
    for app in result["approaches"]:
        ...
        if verification:
            label_counts = Counter(
                item.get("fit_band") for item in verification if item.get("fit_band")
            )
            ...
            for item in verification[: min(analysis_k, 5)]:
                ...
                lines.append(
                    f"- verify {item.get('fit_band')} {item.get('confidence_band')} | {item.get('title')} | {evidence}"
                )
...
async def _run_for_resume(resume_path: Path, args) -> dict[str, Any]:
    ...
    stage_timings_ms["deterministic_profile"] = round(
        (time.perf_counter() - parse_started) * 1000, 2
    )
    llm_enabled = bool(os.environ.get("OPENROUTER_API_KEY"))
    async with (
        httpx.AsyncClient() if llm_enabled else _null_async_client()
    ) as llm_client:
        enrich_started = time.perf_counter()
        enriched_profile = await _enrich_profile_with_llm_with_client(
            base_profile, resume_text, llm_client
        )
        stage_timings_ms["llm_enrich_profile"] = round(
            (time.perf_counter() - enrich_started) * 1000, 2
        )
        ...
        if llm_enabled and any(spec.llm_parse for spec in APPROACHES.values()):
            try:
                ...
                llm_profile = await _llm_parse_resume(
                    resume_text, llm_client, FAST_MODELS[0]
                )
                stage_timings_ms["llm_parse_profile"] = round(
                    (time.perf_counter() - llm_parse_started) * 1000, 2
                )
            except Exception:
                llm_profile = {}
        if args.scrape_jobspy:
            ...
            stage_timings_ms["scrape_jobspy"] = round(
                (time.perf_counter() - scrape_started) * 1000, 2
            )
        else:
        ...
        scored_by_approach: list[
            tuple[ApproachSpec, dict[str, Any], list[dict[str, Any]]]
        ] = []
        for spec in APPROACHES.values():
            ...
            stage_timings_ms[f"rank_{spec.name}"] = round(
                (time.perf_counter() - rank_started) * 1000, 2
            )
        ...
        if args.llm_verify_top:
            ...
            verification = await _llm_verify_top_jobs(
                unique_jobs, resume_text, verification_model, llm_client
            )
            stage_timings_ms["llm_verify_unique_top_jobs"] = round(
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Standalone resume ranker baseline with no DB dependency"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="run optional LLM verification on top N ranked jobs",
    )
    parser.add_argument(
        "--llm-verify-model", default=FAST_MODELS[0], help="model for LLM verification"
    )
    return parser
...
async def main_async() -> None:
    ...
    (out_dir / "summary.md").write_text(
        _render_summary(args.label or resume_path.stem, report, args.analysis_k)
    )
    print(
        json.dumps(
            {
